...
rgb=True
if rgb:
    train_dataset = SegmentationDataset(train_data, transform=transform, grayscale=False)
...
model = SegNet(inch=3, outch=1).to(device)
...
for epoch in range(num_epochs):
    ...
    if epoch_valid_loss < best_loss:
        ...
        torch.save(model.state_dict(), './records/best_segnet_{}.pth'.format(epoch))

np.save('./losses/train_loss_segnet.npy', train_losses)
np.save('./losses/valid_loss_segnet.npy', valid_losses)

train_loss = np.load('./losses/train_loss_segnet.npy', allow_pickle=True)
train_loss = train_loss.tolist()
valid_loss = np.load('./losses/valid_loss_segnet.npy', allow_pickle=True)
...
plt.show()
plt.savefig("./losses/segnet_result.png")
