    ...
    if draw_mask:
        ...
        for object_prediction in object_prediction_list:
            ...
            if object_prediction.mask is not None:
                ...
                # draw mask
                rgb_mask = apply_color_mask(mask, color)
                smooth_rgb_mask = cv2.GaussianBlur(rgb_mask, (3, 3), 0)

                image = cv2.addWeighted(image, 1, smooth_rgb_mask, mask_threshold, 0)
                # image = cv2.addWeighted(image, 1, rgb_mask, mask_threshold, 0)
    if draw_bbox:
        # add bboxes to image if present
        for object_prediction in object_prediction_list:
            # deepcopy object_prediction_list so that original is not altered
...
def visualize_object_predictions_new_unet(
    image: np.array,
    object_prediction_list,
    model,
    device,
    ...
    non_zero_pixels_threshold: int=2500,
):
    image = copy.deepcopy(image)
    ...
    # set text_size for category names
    # text_size = text_size or rect_th / 3
    return_binary_mask=None
    if draw_mask:
        # add masks to image if present
        for o_idx,object_prediction in enumerate(object_prediction_list):
            # deepcopy object_prediction_list so that original is not altered
            object_prediction = object_prediction.deepcopy()
            # visualize masks if present
            if object_prediction.mask is not None:
                # deepcopy mask so that original is not altered
                bbox = object_prediction.bbox.to_xyxy()
                record_position = object_prediction.bbox.to_xyxy()
                _,_,record_bbox_height,record_bbox_width=object_prediction.bbox.to_xywh()
                ...
                x_min, y_min = point1
                x_max, y_max = point2 
                x_min = max(x_min - pad_size, 0)  # Ensure x_min doesn't go below 0
                y_min = max(y_min - pad_size, 0)  # Ensure y_min doesn't go below 0
                x_max = min(x_max + pad_size, image_size[1])  # Ensure x_max doesn't go beyond image width
                y_max = min(y_max + pad_size, image_size[0])  # Ensure y_max doesn't go beyond image h
                ...
                with torch.no_grad():  # Disable gradient calculation since we're in inference mode
                ...
                if x_max > x_min and y_max > y_min:
                    ...
                    y_max +=5
                    y_min -=5
                    x_min = max(x_min, 0)  # Ensure x_min doesn't go below 0
                    y_min = max(y_min, 0)  # Ensure y_min doesn't go below 0
                    x_max = min(x_max, image_size[1])  # Ensure x_max doesn't go beyond image width
                    y_max = min(y_max, image_size[0])
                    print(f"Invalid dimensions for resizing: ensure x_max > x_min and y_max > y_min pad 5 pixel for the mask")
                    print(f"{point1} {point2}x _min: {x_min}, x_max: {x_max}, y_min: {y_min}, y_max: {y_max}")

                    binary_mask_image = cv2.resize(binary_mask, (x_max - x_min, y_max - y_min), interpolation=cv2.INTER_LINEAR)
                    # raise ValueError("Invalid dimensions for resizing: ensure x_max > x_min and y_max > y_min")


                source_mask = np.zeros(image_size[0:2], dtype=bool)
                source_mask[y_min:y_max, x_min:x_max]=binary_mask_image.astype(bool)
                yolo_mask=object_prediction.mask.bool_mask
                yolo_mask_np = (yolo_mask > 0).astype(bool)
                intersection_mask = source_mask & yolo_mask_np
                result_mask_np = intersection_mask.astype(np.uint8)
                # result_mask_np = source_mask.astype(np.uint8)
                non_zero_pixels = np.count_nonzero(result_mask_np)

                if non_zero_pixels<=non_zero_pixels:
                    rgb_mask = apply_color_mask(result_mask_np, color)
                    return_binary_mask=result_mask_np
                    image = cv2.addWeighted(image, 1, rgb_mask, mask_threshold, 0)
                    if draw_text:
                        cv2.putText(image,    f"{o_idx}",    
                                    (int(x_min+(x_max-x_min)/2)-1, int(y_min+(y_max-y_min)/2)-1),  # Position (x, y)
                                    0,                   # Font type (Hershey Simplex)
                                    0.2,           # Text size
                                    (0, 0, 255),         # Blue color (BGR format)
                                    thickness=1    # Text thickness
                                    )
                        output_files.append((o_idx,record_position,record_bbox_width,record_bbox_height,non_zero_pixels))
    if draw_bbox:
        # add bboxes to image if present
        for object_prediction in object_prediction_list:
            # deepcopy object_prediction_list so that original is not altered
        ...
        # image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    return image,output_files,return_binary_mask
    
def merge_all_masks(object_prediction_list, image,draw_mask_threshold=0.6, color=None):
    # 初始化一个总掩码，大小与图像相同
    merged_mask = np.zeros(image.shape[:2], dtype=np.uint8)

    # 遍历每个预测对象
    for object_prediction in object_prediction_list:
        # 深拷贝 mask 并转换为 uint8 类型
        if object_prediction.mask is not None:
            mask = object_prediction.mask.bool_mask.astype(np.uint8)
            # 合并到总掩码中
            merged_mask |= mask
    ...
    if not color:
        color = (255, 0, 0)  # 蓝色
    rgb_mask = apply_color_mask(merged_mask, color)

    # 将合并后的掩码绘制到图像上
    image = cv2.addWeighted(image, 1, rgb_mask, draw_mask_threshold, 0)
